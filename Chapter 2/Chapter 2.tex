...
\begin{document}
	...
	\begin{answered}
		...
		\[\phi(E(X))\leq E(\phi(X))\] We note that $f(x)=x\log(x)$ is a convex function, and so given any p.d.f $\psi$, we have 
		\[E(\psi)\log(E(\psi))\leq E(\psi\log(\psi))\] Multiplying through by -1, and noting that $E(\psi)=\frac{1}{b-a}$, we directly obtain
